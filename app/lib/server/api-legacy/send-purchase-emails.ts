...
import { transformS3UrlToDomain } from './utils/url-transform';
import { inlineEmailCSS } from './utils/email-inline-css';
import { generateCustomerConfirmationEmail, generateAdminNotificationEmail } from './utils/email-templates';
import { getOrderByStripeSession, upsertCustomer, createCard } from './utils/supabase';
...
async function uploadContactCardToS3(sessionId: string, configuration: any, cardNumber: number, req?: Request) {
  try {
    const baseUrl = req ? getBaseUrlFromRequest(req) : getBaseUrlFromRequest(new Request("http://localhost"));
...
async function sendAdminNotificationEmail(session: any, customerData: any, item: any, cardNumber: any, s3Data: any = null, req?: Request) {
  try {
    ...
    if (logoImageData) {
      try {
        const baseUrl = req ? getBaseUrlFromRequest(req) : getBaseUrlFromRequest(new Request("http://localhost"));
        const cardDesignZipResponse = await fetch(`${baseUrl}/api/upload-logo-zip`, {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json',
          },
