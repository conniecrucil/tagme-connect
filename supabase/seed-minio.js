...
async function seedMinIO() {
  ...
  for (const customer of customers) {
    ...
    const htmlContent = generateHTML(customer);
    await uploadToMinIO(`${customer.uuid}/index.html`, htmlContent, 'text/html');
    
    // Create fake images if needed
    if (customer.hasLogo) {
      const logoContent = createFakeImage(200, 200, 'LOGO');
      await uploadToMinIO(`${customer.uuid}/logo.png`, logoContent, 'image/svg+xml');
    }
    
    if (customer.hasPhoto) {
      const photoContent = createFakeImage(300, 300, customer.name.charAt(0));
      await uploadToMinIO(`${customer.uuid}/photo.jpg`, photoContent, 'image/svg+xml');
    }
    
    if (customer.hasCover) {
      const coverContent = createFakeImage(800, 400, 'COVER');
      await uploadToMinIO(`${customer.uuid}/cover.jpg`, coverContent, 'image/svg+xml');
    }
  }
  
  console.log(`\n🎉 MinIO seeding completed!`);
  console.log(`\n📱 Access your seeded cards at:`);
  customers.forEach(customer => {
    console.log(`   http://localhost:9010/${customer.uuid}/index.html`);
  });
}
